...
function boss:OnDestroy()
  EntitiesToKill = EntitiesToKill - 1
  PlayerGun = "Ak47"
  print(PlayerGun)
  local player = Ecs:GetEntity(playerID)
  local holderID = player.holder
  local weapon = Ecs:GetEntity(player.weapon)
  local wx, wy = GetEntityPos(weapon)
  local wrot = GetEntityRotation(weapon)
  DestroyEntity(weapon)
  weaponID = SpawnEntity(PlayerGun, wx, wy, wrot)
